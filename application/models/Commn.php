...
class Commn extends CI_Model
{
    ...
    public function group_by_capitation(){
        $this->db->select('g.name, COUNT(DISTINCT(p.id)) AS total_person_count');
        $this->db->from('gam g');
        $this->db->join('user_membership_plan u', 'g.id = u.gam_id', 'left');
        $this->db->join('hrms_member_of_user_home p', 'u.id = p.member_user_id');
        $this->db->group_by('g.id, g.name');
        return $this->db->get()->result(); 
    }
    public function group_by_capitation_(){
        $this->db->select('g.name, COUNT(DISTINCT(p.id)) AS total_person_count');
        $this->db->from('hrms_member_of_user_home p');
        $this->db->join('user_membership_plan u', 'u.id = p.member_user_id');
        $this->db->join('gam g', 'g.id = u.gam_id', 'left');
        $this->db->group_by('g.id, g.name');
